...
import frc.robot.utils.ChassisAccel;
...
public class Drivetrain extends SubsystemBase {
  ...
  private SlewRateLimiter m_slewX = new SlewRateLimiter(DriveConstants.kDirectionSlewRate);
  private SlewRateLimiter m_slewY = new SlewRateLimiter(DriveConstants.kDirectionSlewRate);
  private SlewRateLimiter m_slewRot = new SlewRateLimiter(DriveConstants.kRotationalSlewRate);

  private final SwerveModule m_FLModule = new SwerveModule(DriveConstants.kFrontLeftDrivingCanId, DriveConstants.kFrontLeftTurningCanId, 0);
  private final SwerveModule m_FRModule = new SwerveModule(DriveConstants.kFrontRightDrivingCanId, DriveConstants.kFrontRightTurningCanId, 0);
  private final SwerveModule m_RLModule = new SwerveModule(DriveConstants.kRearLeftDrivingCanId, DriveConstants.kRearLeftTurningCanId, 0);
  private final SwerveModule m_RRModule = new SwerveModule(DriveConstants.kRearRightDrivingCanId, DriveConstants.kRearRightTurningCanId, 0);

  private static AHRS ahrs = new AHRS(AHRS.NavXComType.kMXP_SPI);
  ...
  private SwerveModuleState[] m_desStates = DriveConstants.kDriveKinematics
  ...
  public void periodic() {
    ...
    m_driveAccel = new ChassisAccel(m_speeds, m_lastDriveSpeed, 0.020);
  ...
  public ChassisSpeeds getChassisSpeed() {
    return DriveConstants.kDriveKinematics.toChassisSpeeds(m_FLModule.getState(), m_FRModule.getState(),
        m_RLModule.getState(),
        m_RRModule.getState());
  }

  public ChassisSpeeds getCorDesChassisSpeed() {
    return DriveConstants.kDriveKinematics.toChassisSpeeds(m_desStates[0], m_desStates[1], m_desStates[2],
        m_desStates[3]);
  }
